...
@section('title', 'Editar Patrimônio - Gestão de Patrimônio')

@section('breadcrumb', 'Editar Patrimônio')

@section('content')
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Editar Patrimônio</h1>
        <p class="text-gray-600 dark:text-gray-400">Atualize os dados do patrimônio</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <!-- Form Content -->
        <form action="{{ route('patrimonios.update', $patrimonio) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="codigo_identificacao" value="{{ old('codigo_identificacao') ?? $patrimonio->codigo_identificacao }}" placeholder="Ex: PAT-2025-001" required class="w-full px-4 py-2 rounded-lg border {{ $errors->has('codigo_identificacao') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 {{ $errors->has('codigo_identificacao') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                ...
                <div>
                    ...
                    <textarea name="descricao" placeholder="Descreva detalhadamente o patrimônio..." rows="4" required class="w-full px-4 py-2 rounded-lg border {{ $errors->has('descricao') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 {{ $errors->has('descricao') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }} resize-none">{{ old('descricao') ?? $patrimonio->descricao }}</textarea>
                    @error('descricao')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <select name="categoria_id" required class="w-full px-4 py-2 rounded-lg border {{ $errors->has('categoria_id') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 {{ $errors->has('categoria_id') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                            ...
                            @foreach($categorias as $categoria)
                                <option value="{{ $categoria->id }}" {{ (old('categoria_id') ?? $patrimonio->categoria_id) == $categoria->id ? 'selected' : '' }}>{{ $categoria->nome }}</option>
                            @endforeach
                        </select>
                        @error('categoria_id')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="localizacao_id" required class="w-full px-4 py-2 rounded-lg border {{ $errors->has('localizacao_id') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 {{ $errors->has('localizacao_id') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                            ...
                            @foreach($localizacoes as $localizacao)
                                <option value="{{ $localizacao->id }}" {{ (old('localizacao_id') ?? $patrimonio->localizacao_id) == $localizacao->id ? 'selected' : '' }}>{{ $localizacao->nome }} - {{ $localizacao->setor }}</option>
                            @endforeach
                        </select>
            ...
            <!-- Section 3: Responsabilidade e Fornecedor -->
            <div>
                    ...
                    <div>
                        ...
                        <select name="usuario_responsavel_id" class="w-full px-4 py-2 rounded-lg border {{ $errors->has('usuario_responsavel_id') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 {{ $errors->has('usuario_responsavel_id') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                            ...
                            @foreach($usuarios as $usuario)
                                <option value="{{ $usuario->id }}" {{ (old('usuario_responsavel_id') ?? $patrimonio->usuario_responsavel_id) == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="fornecedor_id" class="w-full px-4 py-2 rounded-lg border {{ $errors->has('fornecedor_id') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 {{ $errors->has('fornecedor_id') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                            ...
                            @foreach($fornecedores as $fornecedor)
                                <option value="{{ $fornecedor->id }}" {{ (old('fornecedor_id') ?? $patrimonio->fornecedor_id) == $fornecedor->id ? 'selected' : '' }}>{{ $fornecedor->nome }}</option>
                            @endforeach
                        </select>
                        @error('fornecedor_id')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        @php
                            $dataValue = '';
                            if (old('data_aquisicao')) {
                                $dataValue = old('data_aquisicao');
                            } elseif ($patrimonio->data_aquisicao) {
                                $dataValue = is_string($patrimonio->data_aquisicao) ? $patrimonio->data_aquisicao : $patrimonio->data_aquisicao->format('Y-m-d');
                            }
                        @endphp
                        <input type="date" name="data_aquisicao" value="{{ $dataValue }}" class="w-full px-4 py-2 rounded-lg border {{ $errors->has('data_aquisicao') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 {{ $errors->has('data_aquisicao') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                    ...
                    <div>
                        ...
                        <input type="number" name="valor" value="{{ old('valor') ?? $patrimonio->valor }}" placeholder="0.00" step="0.01" min="0" class="w-full px-4 py-2 rounded-lg border {{ $errors->has('valor') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 {{ $errors->has('valor') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                        @error('valor')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="estado" required class="w-full px-4 py-2 rounded-lg border {{ $errors->has('estado') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 {{ $errors->has('estado') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }}">
                            <option value="bom" {{ (old('estado') ?? $patrimonio->estado) == 'bom' ? 'selected' : '' }}>Bom</option>
                            <option value="novo" {{ (old('estado') ?? $patrimonio->estado) == 'novo' ? 'selected' : '' }}>Novo</option>
                            <option value="regular" {{ (old('estado') ?? $patrimonio->estado) == 'regular' ? 'selected' : '' }}>Regular</option>
                            <option value="ruim" {{ (old('estado') ?? $patrimonio->estado) == 'ruim' ? 'selected' : '' }}>Ruim</option>
                            <option value="inservivel" {{ (old('estado') ?? $patrimonio->estado) == 'inservivel' ? 'selected' : '' }}>Inservível</option>
                        </select>
                        @error('estado')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div>
                ...
                <div>
                    ...
                    <textarea name="observacoes" placeholder="Adicione observações sobre o patrimônio..." rows="4" class="w-full px-4 py-2 rounded-lg border {{ $errors->has('observacoes') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 {{ $errors->has('observacoes') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }} resize-none">{{ old('observacoes') ?? $patrimonio->observacoes }}</textarea>
                    @error('observacoes')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
