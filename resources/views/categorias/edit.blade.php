...
@section('title', 'Editar Categoria - Gestão de Patrimônio')

@section('breadcrumb', 'Editar Categoria')

@section('content')
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Editar Categoria</h1>
        <p class="text-gray-600 dark:text-gray-400">Atualize os dados da categoria</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <form action="{{ route('categorias.update', $categoria) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="descricao" placeholder="Descreva a categoria..." rows="4" class="w-full px-4 py-2 rounded-lg border {{ $errors->has('descricao') ? 'border-red-500 dark:border-red-400' : 'border-gray-300 dark:border-gray-600' }} bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 {{ $errors->has('descricao') ? 'focus:ring-red-500 dark:focus:ring-red-400' : 'focus:ring-blue-900 dark:focus:ring-blue-400' }} resize-none">{{ old('descricao') ?? $categoria->descricao }}</textarea>
                @error('descricao')
                    <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
